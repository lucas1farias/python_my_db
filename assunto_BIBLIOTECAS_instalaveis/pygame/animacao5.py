...
No arquivo seguinte: adição de inimigo em local específico
"""
...
class Enemy:
    ...
    def draw(self):
        # TODO: O desenho depende do inimigo escolhido
    ...
    def update(self):
        ...
        # TODO: Por causa dos "else". A administração das imagens também precisa ser separada
        if self.enemy_name == 'shell_purple':
    ...
    def move(self, direction, this_speed):
        # TODO: A movimentação horizontal dos sprites precisam ser diferenciadas
    ...
    def __init__(self, enemy_name, random_location=False, location=0):
        # TODO: Como haverá mais de um inimigo, é preciso adicionar "enemy_name"
        ...
        # TODO: Cada novo objeto receberá uma nova posição horizontal
        self.shell_locations = [*range(700, 2001)]
        self.current_sprite = 0

        # TODO: Para criação múltipla, as superfícies e retângulos precisam estar em condições para serem diferenciadas
        if enemy_name == 'shell_purple':

            # TODO: Imagens convertidas e redimensionadas, inseridas num retângulo da tamanho da imagem
            ...
            # TODO: O sprite é criado numa localização aleatória
            ...
            # TODO: O sprite é colocado numa posição proposital
            else:
                self.x = location
                self.shell_purple_rectangles = [
                    self.shell_purple_surfaces[0].get_rect(topleft=(self.x, self.y)),
...
# TODO: Atributos necessários para criação de inimigos (a cada novo sprite, uma novo string em "shell_types")
shell_types = ['shell_purple', 'shell_green']
shell_amount = 10
shell_box = []


# TODO: Porque esse método está fora da classe "Enemy"? Por envolver criação de objeto, e isso é feito fora da classe
def make_enemy_box(enemy_type, enemy_object_box, end_point, enemy_type_box):
    counter = 0
    if enemy_type == 'shell':
...
# TODO: Criação dos inimigos
make_enemy_box('shell', shell_box, shell_amount, shell_types)

# TODO: Demonstração do grupo de inimigos: tipo e localização horizontal
for shell in shell_box:
    print(f'{shell.enemy_name} / {shell.x}')
